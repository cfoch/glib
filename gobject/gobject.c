               ...
               const GValue   values[])
{
  ...
  if (n_properties > 0)
    {
      gint n;
      GObjectConstructParam params[n_properties];

      for (n = 0; n < n_properties; n++) {
        GParamSpec *pspec;
        pspec = g_param_spec_pool_lookup (pspec_pool, names[n], object_type, TRUE);
        if (!g_object_new_is_valid_property (object_type, pspec, names[n],
            params, n))
          break;
        params[n].pspec = pspec;

        /* Init GValue */
        params[n].value = g_newa (GValue, 1);
        memset (params[n].value, 0, sizeof (GValue));
        g_value_init (params[n].value, G_VALUE_TYPE (&values[n]));

        g_value_copy (&values[n], params[n].value);
      }
      object = g_object_new_internal (class, params, n);
      while (n--)
  ...
  for (n = 0; n < n_properties; n++)
    {
      ...
      if (g_object_set_is_valid_property (object, pspec, names[n]))
        {
          consider_issuing_property_deprecation_warning (pspec);
          object_set_property (object, pspec, &values[n], nqueue);
        }
    }
  ...
  for (n = 0; n < n_properties; n++)
    {
      pspec = g_param_spec_pool_lookup (pspec_pool,
				        ...
				        G_OBJECT_TYPE (object),
				        TRUE);
      if (!g_object_get_is_valid_property (object, pspec, names[n]))
        break;

      memset (&values[n], 0, sizeof (GValue));
      g_value_init (&values[n], pspec->value_type);
      object_get_property (object, pspec, &values[n]);
    }
  g_object_unref (object);
}
